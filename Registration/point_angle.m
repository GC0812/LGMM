...
seedx = 1 + (seed - seedy)/M; %seed（最佳中心位置）的行数和列数
starty = mod(start, M); 
if (starty==0) starty = M; end
startx = 1 + (start - starty)/M; %中心位置的8邻域有值的那个位置的行数和列数

dy = 1 + R + starty - seedy;
dx = 1 + R + startx - seedx; 

% set the region border from 1:8*R
region = zeros(2*R+1); %初始化一个7*7区域
region(R+1:-1:1,end) = (1:R+1)'; 
region(1,end-1:-1:2)= R+2:3*R;
region(1:end,1)= (3*R+1:5*R+1)';
region(end,2:end-1) = 5*R+2:7*R;
region(end:-1:R+2,end) = (7*R+1:8*R)'; %给边界编号，从三点钟方向开始逆时针绕一周（1-24）
