...
if isempty(linktableoriginal1) || isempty(linktableoriginal2)
    ...
    if(~isempty(loop1_3)&&~isempty(loop2_3))
        ...
        if Pidx1~=1
            matchingpair2(1).points=circshift(matchingpair2(1).points, [0, (Pidx1(1)-1)*(5+2)]);
        end
    else
    ...
    if(~isempty(loop1_4)&&~isempty(loop2_4))
        ...
        if Pidx2~=1
            matchingpair2(2).points=circshift(matchingpair2(2).points, [0, (Pidx2(1)-1)*(5+2)]);
        end
    else
    ...
    if(~isempty(loop1_5)&&~isempty(loop2_5))
        ...
        if Pidx3~=1
            matchingpair2(3).points=circshift(matchingpair2(3).points, [0, (Pidx3(1)-1)*(5+2)]);
        end
    else
    ...
    if ~isempty(matchingpair1(1).points) || ~isempty(matchingpair1(2).points) || ~isempty(matchingpair1(3).points)
        [points_bb,points_aa,outbw2,mytform]=global_registration(bw1,bw2,matchingpair1,matchingpair2,linktableoriginal1,linktableoriginal2);%环-血管配准
        [outmosaicbw_twice,sae_twice,mytform_twice,flag]=local_registration(bw1,bw2,mytform,outbw2,points_bb,points_aa,ptbifu1,ptbifu2);%环-血管-分叉点配准
    else
