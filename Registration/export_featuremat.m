function [ featuremat3,featuremat4,featuremat5,loop3,loop4,loop5,linktableoriginal] = export_featuremat( bw )
...
for i=1:size(loopa,1)
    temp = verifystucture( linktable1,loopa(i,:),path_points );
    if(~isempty(temp))
...
for i=1:size(loopb,1)
    temp = verifystucture( linktable1,loopb(i,:),path_points );
    if(~isempty(temp))
...
for i=1:size(loopc,1)
    temp = verifystucture( linktable1,loopc(i,:),path_points );
    if(~isempty(temp))
